...
export class PrimitiveMesh {
	 ...
	 * 总点数=水平层数*垂直层数*2
	 */
	static createMyCylinder(radius: number = 0.5, height: number = 2, h_slices: number = 200, slices: number = 32): Mesh {
		//(this._released) || (dispose());//如果已存在，则释放资源
		let Y = h_slices + 1
		let X = slices + 1
		var vertexCount: number =  X * Y * 2;// + (X + 1);
		var indexCount: number = 3 * 2 * h_slices * slices * 2;//  + 3 * slices;
		...
		//----------------------------- 侧面---------------------------------
		for (let l_index: number = 0; l_index <= slices; l_index++) {
			curAngle = l_index * sliceAngle;
			posX = Math.cos(curAngle) * radius;
			posZ = Math.sin(curAngle) * radius;

			//h位置
			for (let h_index: number = 0; h_index <= h_slices; h_index++) {
				posY = height * h_index / h_slices;
				// 画顶点
				//pos(X,Y,Z)
				vertices[vc++] = posX;
				vertices[vc++] = posY;
		...
		// //(this._released) || (dispose());//如果已存在，则释放资源
		// // 以下注释，假定圆平面切分为 32份,柱子竖向切分为 200份
		// let Y = h_slices + 1 	// 	Y点数
		// let X = l_slices 		//  X点数

		// var vertexCount: number = X * Y + (l_slices + 1 + 1) *2
		// var indexCount: number = 3 * 2 * h_slices * l_slices + (l_slices) * 3 * 2; 	//面对应的点数（三角形数量*3）
		...
		// var sliceAngle: number = (Math.PI * 2.0) / l_slices;
		...
		// //顶
		// for (var tv: number = 0; tv <= l_slices; tv++) {
		...
		// for (var ti: number = 0; ti < l_slices; ti++) {
		// 	indices[ic++] = 0;
		// 	indices[ic++] = ti + 1;
		// 	indices[ic++] = ti + 2;
		// }
		// verticeCount += l_slices + 1 + 1;		

		// //壁

		// // l位置
		// for (let l_index: number = 0; l_index < l_slices; l_index++) {
		// 	curAngle = l_index * sliceAngle;
		// 	posX = Math.cos(curAngle) * radius;
		// 	posZ = Math.sin(curAngle) * radius;
		...
		// 		//uv    (x,y)
		// 		vertices[vc++] = l_index * 1 / l_slices;
		// 		vertices[vc++] = h_index / h_slices;
		// 	}
		// }
		...
		// 		if (l_index == l_slices - 1) {
		// 			indices[ic++] = verticeCount + l_index * Y + h_index				//(x, y)
		// 			indices[ic++] = verticeCount + l_index * Y + h_index + 1			//(x, y+1)
		// 			indices[ic++] = verticeCount + h_index + 1							//(0, y+1)

		// 			indices[ic++] = verticeCount + l_index * Y + h_index				//(x,y)
		// 			indices[ic++] = verticeCount + h_index + 1 						//(0, y+1)
		// 			indices[ic++] = verticeCount + h_index								//(0, y)
